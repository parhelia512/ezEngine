...
namespace ezInternal
{
  ...
  template <typename T>
  EZ_FORCE_INLINE void Delete(ezAllocatorBase* pAllocator, T* pPtr)
  {
    if (pPtr != nullptr)
    {
  ...
  EZ_FORCE_INLINE void DeleteRawBuffer(ezAllocatorBase* pAllocator, void* pPtr)
  {
    if (pPtr != nullptr)
    {
      pAllocator->Deallocate(pPtr);
  ...
  template <typename T>
  EZ_FORCE_INLINE T* ExtendRawBuffer(T* pPtr, ezAllocatorBase* pAllocator, size_t uiCurrentCount, size_t uiNewCount, ezTypeIsPod)
  {
    return (T*)pAllocator->Reallocate(pPtr, uiCurrentCount * sizeof(T), uiNewCount * sizeof(T), EZ_ALIGNMENT_OF(T));
  }

  template <typename T>
  EZ_FORCE_INLINE T* ExtendRawBuffer(T* pPtr, ezAllocatorBase* pAllocator, size_t uiCurrentCount, size_t uiNewCount, ezTypeIsMemRelocatable)
  {
    return (T*)pAllocator->Reallocate(pPtr, uiCurrentCount * sizeof(T), uiNewCount * sizeof(T), EZ_ALIGNMENT_OF(T));
  }

  template <typename T>
  EZ_FORCE_INLINE T* ExtendRawBuffer(T* pPtr, ezAllocatorBase* pAllocator, size_t uiCurrentCount, size_t uiNewCount, ezTypeIsClass)
  {
  ...
  template <typename T>
  EZ_FORCE_INLINE T* ExtendRawBuffer(T* pPtr, ezAllocatorBase* pAllocator, size_t uiCurrentCount, size_t uiNewCount)
  {
