...
#include <GameEngine/AI/UtilityAI/Impl/GameAiActions.h>
...
ezAiActionResult ezAiActionWait::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
  if (tDiff >= m_Duration)
...
ezAiActionResult ezAiActionLerpRotation::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
...
ezAiActionResult ezAiActionLerpPosition::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
...
ezAiActionResult ezAiActionLerpRotationTowards::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
  if (m_TurnAnglesPerSec <= ezAngle())
    return ezAiActionResult::Finished; // or canceled
...
//}
//
// ezAiActionResult ezAiActionFollowPath::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
//{
...
ezAiActionResult ezAiActionBlackboardSetEntry::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
  ...
  {
    ezLog::Error(pLog, "ezAiActionBlackboardSetEntry: Blackboard entry '{}' doesn't exist.", m_sEntryName.GetHash());
    return ezAiActionResult::Failed;
  }
...
ezAiActionResult ezAiActionBlackboardWait::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
  if (m_sEntryName.IsEmpty())
    return ezAiActionResult::Finished; // or canceled
...
ezAiActionResult ezAiActionBlackboardSetAndWait::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
  if (m_sEntryName.IsEmpty())
    return ezAiActionResult::Finished; // or canceled
  ...
  if (m_SetValue.IsValid())
  {
    ...
    {
      ezLog::Error(pLog, "ezAiActionBlackboardSetAndWait: Blackboard entry '{}' doesn't exist.", m_sEntryName.GetHash());
      return ezAiActionResult::Failed;
    }
...
ezAiActionResult ezAiActionCCMoveTo::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
  if (m_fSpeed <= 0.0f)
    return ezAiActionResult::Finished; // or canceled
...
ezAiActionResult ezAiActionSpawn::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
...
{
  inout_sText.Format("Log: {}", m_sLogMsg);
}

ezAiActionResult ezAiActionQuip::Execute(ezGameObject* pOwner, ezTime tDiff, ezLogInterface* pLog)
{
...
void ezAiActionQuip::Cancel(ezGameObject* pOwner)
{
  Reset();
}
